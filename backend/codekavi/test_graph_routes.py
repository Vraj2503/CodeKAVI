...
passthrough, 404 for unknown repos. GET .../tour/diff (H3): 404 when a cached
result predates last_change_map, IDOR denial on owner mismatch. See
docs/superpowers/plans/2026-07-25-graph-phase1-plan.md Step 5 and
Order of implementation.txt F5 H5."""
...
def _result(owner_user_id: str | None = "user-a", last_change_map: dict | None = None) -> dict:
    result = {
    ...
    if last_change_map is not None:
        result["last_change_map"] = last_change_map
    return result
...
# ─────────────────────────────────────────
# H3/H5: GET /graph/semantic/{repo_id}/tour/diff
# ─────────────────────────────────────────


def test_diff_tour_returns_200_with_ordered_steps():
    change_map = {"api/routes/a.py": "STRUCTURAL", "api/routes/b.py": "COSMETIC"}
    client = _client({"repo1": _result(last_change_map=change_map)})
    resp = client.get("/api/graph/semantic/repo1/tour/diff")

    assert resp.status_code == 200
    body = resp.json()
    assert body["mode"] == "diff"
    assert [s["node_ids"][0] for s in body["steps"]] == ["api/routes/a.py", "api/routes/b.py"]
    assert body["deleted_count"] == 0


def test_diff_tour_missing_last_change_map_returns_404_no_prior_analysis():
    """A cached result that predates last_change_map (analyzed before this
    feature shipped) must 404 distinctly from "nothing changed" — the latter
    is a present, possibly-empty map and a 200 with zero steps."""
    client = _client({"repo1": _result(last_change_map=None)})
    resp = client.get("/api/graph/semantic/repo1/tour/diff")

    assert resp.status_code == 404
    assert "diff tour" in resp.json()["detail"].lower()


def test_diff_tour_unknown_repo_id_returns_404():
    client = _client({})
    resp = client.get("/api/graph/semantic/does-not-exist/tour/diff")
    assert resp.status_code == 404


def test_diff_tour_cross_user_access_returns_404_not_403():
    change_map = {"api/routes/a.py": "STRUCTURAL"}
    client = _client(
        {"repo1": _result(owner_user_id="user-a", last_change_map=change_map)},
        user_id="user-b",
    )
    resp = client.get("/api/graph/semantic/repo1/tour/diff")
    assert resp.status_code == 404
