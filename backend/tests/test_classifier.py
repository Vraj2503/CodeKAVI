...
class TestRoleDetection:
    ...
    def test_entry_point_detected(self, sample_file_profiles):
        ...
        assert main_profile["role"] == "entry_point", f"main.py should be entry_point, got: {main_profile['role']}"
    ...
    def test_config_detected(self, sample_file_profiles):
        ...
        assert config_profile["role"] == "config", f"config.py should be config, got: {config_profile['role']}"
    ...
    def test_test_file_detected(self, sample_file_profiles):
        ...
        test_profiles = [p for path, p in profiles_by_path.items() if "test_helpers" in path]
        assert len(test_profiles) > 0, "test_helpers.py should be in profiles"
        assert test_profiles[0]["role"] == "test", f"test_helpers.py should be test, got: {test_profiles[0]['role']}"
    ...
    def test_documentation_detected(self, sample_file_profiles):
        ...
        assert readme["role"] == "documentation", f"README.md should be documentation, got: {readme['role']}"
...
class TestImportanceOrdering:
    ...
    def test_profiles_sorted_by_importance(self, sample_file_profiles):
        ...
        assert scores == sorted(scores, reverse=True), "Profiles should be sorted by importance_score descending"
        ...
        for p in sample_file_profiles:
            if p["role"] == "entry_point":
                assert p["importance_score"] >= 10, (
                    f"Entry point {p['path']} should have importance >= 10, got: {p['importance_score']}"
                )
