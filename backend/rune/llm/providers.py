...
if TYPE_CHECKING:
    from rune.exceptions import ProviderError
...
class CircuitBreaker:
    ...
    def _emit_transition(_from: str, _to: str) -> None:
        ...
        try:
            from rune.metrics import record_breaker_transition
...
def _record_llm_usage(user_id: str | None, provider: str, tokens: int, latency_ms: int) -> None:
    ...
    try:
        ...
        try:
            from rune.metrics import record_llm_usage
...
class GroqProvider:
    ...
    async def _generate_core(
        ...
        from rune.utils import current_io_executor
