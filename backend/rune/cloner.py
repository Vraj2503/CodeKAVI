...
from rune.config import CLONE_BASE_DIR
from rune.repo_source import RepoSourceInfo
...
def parse_github_url(url: str) -> RepoSourceInfo:
    ...
    """
    from rune.repo_source import GitHubSource
...
def parse_repo_url(url: str) -> RepoSourceInfo:
    ...
    """
    from rune.repo_source import detect_source
...
def clone_repo(github_url: str) -> dict:
    ...
    try:
        ...
        if os.path.exists(clone_path):
            shutil.rmtree(clone_path, ignore_errors=True)
        from rune.exceptions import CloneError
