...
export type RFHighlight = "off" | "dim" | "hover" | "trace-up" | "trace-down" | "select";
...
export function toRFNodes(nodes: FlowNode[], expanded: ReadonlySet<string> = new Set()): RFNode[] {
  ...
  const presentKinds = new Set(leafNodes.map(n => n.data.flow.kind as NodeKind));
  ...
  const groupNodes = Array.from(presentKinds).map((kind) => {
    ...
    return {
      ...
      focusable: false,
      data: {
        flow: { id: `category-frame-${kindKey}`, label: KIND_LABEL[kindKey], kind: kindKey },
...
function componentSets(nodes: FlowNode[], edges: FlowEdge[]): FlowNode[][] {
  ...
  for (const edge of edges) {
    if (edge.direction === "response" || !byId.has(edge.source) || !byId.has(edge.target)) continue;
 ...
 */
export function prepareFlowGraph(nodes: FlowNode[], edges: FlowEdge[]): { nodes: FlowNode[]; edges: FlowEdge[] } {
  ...
  const dashboardComponent = components.find((component) => component.some((node) => /dashboard/i.test(node.label)));
  const ordered = components
    .filter((component) => component !== dashboardComponent)
    .sort((a, b) => b.length - a.length || Math.max(...b.map((node) => node.tier ?? 0)) - Math.max(...a.map((node) => node.tier ?? 0)));
  ...
  for (let index = 0; index < ordered.length - 1; index += 1) {
    ...
    const source = current
      .filter((node) => !validEdges.some((edge) => edge.source === node.id && currentIds.has(edge.target)))
      .sort((a, b) => (b.tier ?? 0) - (a.tier ?? 0))[0] ?? current[0];
    const target = next
      .filter((node) => !validEdges.some((edge) => edge.target === node.id && nextIds.has(edge.source)))
  ...
  const incoming = new Set(connectedEdges.filter((edge) => edge.direction !== "response").map((edge) => edge.target));
  const outgoing = new Set(connectedEdges.filter((edge) => edge.direction !== "response").map((edge) => edge.source));
  ...
  for (const node of validNodes) {
    ...
    if (isDashboard) {
      ...
      finalNodes.push({ ...node, id: entryId, label: 'Dashboard', kind: 'start' });
      ...
      else if (!incoming.has(node.id)) finalNodes.push({ ...node, kind: node.kind === "data_store" ? "data_store" : "start" });
...
export function expandTechnologies(
  ...
  return { nodes: [...nodes, ...detailNodes], edges: [...edges, ...detailEdges] };
}
